...
public class ListSessions {
            ...
            public void run() {
                if (args.length <= 3) {
                    if (args.length == 3 && (StringUtils.isNumeric(args[1]) || !StringUtils.isNumeric(args[2]))) {
                        ...
                        ListSessions.usage(p, slash);
                    } else {
                        ListSessions.executeInternally(p, args);
                    }
                } else {
                    p.sendMessage(ChatColor.RED + "Too many arguments.");
                    ListSessions.usage(p, slash);
    ...
    private void executeInternally(final @NotNull Player p, final @NotNull String[] args) {
        ...
        ListSessions.printList(p, page);
    }

    private void printList(final @NotNull Player p, final int page) {
        ...
        final @Nullable java.util.List<String> sessionList = sessions == null ? new GapList<>() : sessions.keySet().stream().filter(name -> !name.equals("current")).collect(Collectors.toList());
        ...
        if (sessionList.isEmpty()) {
            ...
            for (int i = 0; i < listmax; i++) {
                ListSessions.sendListLine(p, id, sessionList.get(id));
                id++;
            }
    ...
    private void usage(final @NotNull Player p, @NotNull final String slash) {
        GlobalMessageUtils.sendSuggestMessage(ChatColor.RED + "Usage: ",
                                              ...
                                              ListSessions.usageCommand(slash), p);
    }
}