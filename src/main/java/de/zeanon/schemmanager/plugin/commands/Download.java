			...
			public void run() {
				if (args.length == 4) {
					...
					if (args.length < 3) {
						...
						if (args[4].equalsIgnoreCase("confirm") && CommandRequestUtils.checkDownloadRequest(p.getUniqueId(), args[3])) {
						...
						} else if (args[3].equalsIgnoreCase("deny") && CommandRequestUtils.checkDownloadRequest(p.getUniqueId(), args[3])) {
							CommandRequestUtils.removeDownloadRequest(p.getUniqueId());
