			...
			public void run() {
				...
				if (args.length == 4) {
					if (oldFileExists) {
						...
						WorldEditVersionRequestUtils.addCopyRequest(p, args[2]);
					} else {
						p.sendMessage(ChatColor.GOLD + args[2] + ChatColor.RED + " does not exist.");
					}
				} else if (args.length == 5 && WorldEditVersionRequestUtils.checkCopyRequest(p, args[2])) {
					if (args[4].equalsIgnoreCase("confirm")) {
						WorldEditVersionRequestUtils.removeCopyRequest(p);
						if (oldFileExists) {
