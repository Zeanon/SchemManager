...
public class DeleteFolder {
				...
				if (args.length <= 5) {
					if (args.length < 3) {
						...
						DeleteFolder.usage(p, slash, schemAlias);
							   ...
							   && !FastAsyncWorldEditModeRequestUtils.checkDeleteFolderRequest(p.getUniqueId(), args[2])
	...
	private void executeInternally(final @NotNull Player p, final @NotNull String @NotNull [] args) {
		...
		if (args.length == 3) {
			if (fileExists) {
				...
				FastAsyncWorldEditModeRequestUtils.addDeleteFolderRequest(p.getUniqueId(), args[2]);
			} else {
				p.sendMessage(ChatColor.DARK_GRAY + "[" + ChatColor.DARK_RED + SchemManager.getInstance().getName() + ChatColor.DARK_GRAY + "] " +
							  ChatColor.GREEN + args[2] + ChatColor.RED + " does not exist.");
			}
		} else if (args.length == 4 && FastAsyncWorldEditModeRequestUtils.checkDeleteFolderRequest(p.getUniqueId(), args[2])) {
	...
	private @Nullable String getParentName(final @NotNull File file) {
		...
		if (ConfigUtils.getBoolean("Delete empty Folders")
			&& !file.getAbsoluteFile().getParentFile().equals(FastAsyncWorldEditModeSchemUtils.getSchemFolder())) {
