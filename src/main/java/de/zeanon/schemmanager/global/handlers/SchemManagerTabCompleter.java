	...
	public List<String> onTabComplete(final @NotNull CommandSender sender, final @NotNull Command command, final @NotNull String alias, final @NotNull String @NotNull [] args) {
		if (args.length == 1) {
			...
			return completions;
										 ...
										 && (RequestUtils.checkUpdateRequest(((Player) sender).getUniqueId())
											 || RequestUtils.checkDisableRequest(((Player) sender).getUniqueId())))
										|| (sender instanceof ConsoleCommandSender
											&& (RequestUtils.checkConsoleUpdateRequest()
												|| RequestUtils.checkConsoleDisableRequest())))) {
			final @NotNull List<String> completions = new GapList<>();
